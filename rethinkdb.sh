...
if [ -z "$REAL_IP" ]; then
	...
	docker run \
		--name rethinkdb \
		-d \
		-p $(/sbin/ifconfig eth1 | grep "inet addr" | awk -F: '{print $2}' | awk '{print $1}'):8080:8080 \
		-p $(/sbin/ifconfig eth1 | grep "inet addr" | awk -F: '{print $2}' | awk '{print $1}'):28015:28015 \
		-p $(/sbin/ifconfig eth1 | grep "inet addr" | awk -F: '{print $2}' | awk '{print $1}'):29015:29015 \
		-v /data/rethinkdb:/data \
		dockerfile/rethinkdb \
		rethinkdb \
			--bind all \
			--canonical-address $REAL_IP \
			--server-name `hostname | sed 's/-/_/g'` \
			$RETHINKDB_JOIN_CMD
fi
